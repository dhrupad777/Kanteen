...
export function OrderCard({ order, role }: OrderCardProps) {
  ...
  const otp = typeof window !== 'undefined' ? localStorage.getItem(`kanteen_otp_${order.id}`) : null;
          ...
          <div className={cn(
            ...
            {role === 'student' && order.status === 'Ready' && order.token && order.token >= 200 && (
              <div className="mt-3 pt-3 border-t border-green-400/30 w-full animate-in fade-in slide-in-from-top-2 duration-500">
                {otp ? (
                  <>
                    <p className="text-[10px] uppercase font-bold tracking-[0.2em] text-green-800/60 dark:text-green-200/60 mb-1">Pick-up OTP</p>
                    <p className="text-2xl md:text-3xl font-mono font-black text-green-900 dark:text-green-100 tracking-[0.1em]">{otp}</p>
                  </>
                ) : (
                  <p className="text-[10px] leading-tight text-green-800/70 dark:text-green-200/70 italic font-medium px-2">OTP will be here when order is ready</p>
                )}
              </div>
            )}
          </div>
